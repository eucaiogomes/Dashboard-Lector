...
import { CCTab } from '../types';
import { costCenterRowsData } from '../data/mockData';
...
/**
 * Exact copy of the "Indicadores por centro de custo" table card from CentroCustoView
        ...
        <div className="divide-y divide-[#f1f3f6] min-w-[960px]">
          {sortedRows.map((r, i) => {
            ...
            return (
              ...
              >
                <div className="font-semibold text-[#004e4c] px-1.5 truncate">
                  {r.area}
                </div>
                ...
                <div className="text-[#4a5462] px-1.5">
                  <div className="font-medium text-[#004e4c] truncate">{r.treinamento}</div>
          ...
          {[1, 2, 3].map(page => (
            <button
              ...
              className={`px-2.5 py-1 rounded-[3px] font-semibold cursor-pointer transition-colors ${
                currentPage === page
                  ? 'border border-[#004e4c] bg-[#004e4c] text-[#eef7f4]'
