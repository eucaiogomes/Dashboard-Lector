...
export const InstitucionaisKpiBlock: React.FC<InstitucionaisKpiBlockProps> = ({ period = 'Agosto - 2026' }) => {
  ...
  return (
    <div className="h-full w-full bg-white rounded-[6px] border border-[#e0e5eb] shadow-2xs p-4 flex flex-col select-none">
      {/* 5 KPIs grid — flex-1 lets the tiles themselves grow into extra card height
          (and recenter their content) instead of leaving a dead gap before the strip below. */}
      <div className="flex-1 min-h-0 grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        {simulatedData.kpis.map((kpi, index) => (
          ...
          >
            <div className="text-[10.5px] uppercase tracking-wider text-[#8a93a0] font-semibold truncate">
              {kpi.label}
            </div>
            <div className="flex items-baseline gap-1">
              <div className="text-[24px] font-bold text-[#004e4c] tracking-tight leading-none">
                {kpi.value}
              </div>
              {kpi.unit && (
            ...
            <div className="text-[11px] text-[#6b7684] truncate">
              {kpi.delta}
            </div>
          </div>
        ))}
      ...
      <div className="mt-3 pt-3 border-t border-[#f0f3f7] flex flex-wrap items-center justify-between gap-4 shrink-0">
        ...
        <div className="flex items-center gap-6 text-right">
          ...
          <div>
            <div className="text-[18px] font-bold text-[#f47920] leading-tight">
              {esforcoPct}%
            </div>
            <div className="text-[10.5px] text-[#8a93a0] font-medium">
              Esforço extra
